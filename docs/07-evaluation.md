...
Side-by-side quality eval: same docs through LakeAudit's loop and through single-pass GPT-4, scored blind by a Wafer-hosted judge model. The answer to "are the labels actually useful?"

## Scope

- **~200 held-out docs** drawn from the same arXiv + NSF corpus, sampled for content-type and discipline diversity.
- **Separate CLI subcommand**: `lakeaudit eval --n 200 --run-id <id>`. Independent of `lakeaudit run`; can run before or after the main corpus loop.
- **The only place GPT-4 actually executes end-to-end.** Real OpenAI API spend, ~$1–5 per doc baseline depending on length.

## Pair construction (`lakeaudit/eval/harness.py`)

For each held-out doc:

1. Run LakeAudit's full 6-pass loop → produces a record matching the `RefinedRecord + EnrichedPayload` shape (see [`03`](03-prompts-and-schemas.md)).
...
3. Insert into `eval_pairs(doc_id, lakeaudit_record, gpt4_record, a_is_lakeaudit)`.

`a_is_lakeaudit` is randomized per pair (50/50). Stored separately from the judge call so the judge never sees the mapping.
...
import random
a_is_lakeaudit = random.random() < 0.5
record_a = lakeaudit_record if a_is_lakeaudit else gpt4_record
record_b = gpt4_record if a_is_lakeaudit else lakeaudit_record
```

Judge receives `(record_a, record_b, document_text)`. It returns `winner: A | B | tie`. Scoring code joins with `a_is_lakeaudit` to attribute the win.
...
Aggregate: weighted sum. Weights default to equal but tunable in `lakeaudit/eval/scoring.py` (e.g., upweight `compliance_correctness` since false negatives there are the most expensive type of error).

## Scoring math (`lakeaudit/eval/scoring.py`)

After all eval results in:

```python
n_lakeaudit_wins = count(winner == "lakeaudit")
n_gpt4_wins      = count(winner == "gpt4")
n_ties           = count(winner == "tie")
win_rate         = n_lakeaudit_wins / (n_lakeaudit_wins + n_gpt4_wins)

# per-dimension delta
for dim in DIMENSIONS:
    lakeaudit_avg = mean(scores[pair][dim].lakeaudit for pair in pairs)
    gpt4_avg      = mean(scores[pair][dim].gpt4 for pair in pairs)
    delta[dim]    = lakeaudit_avg - gpt4_avg
...
- JSON report at `./.lakeaudit/eval_report_<run_id>.json`.
- Dashboard panel 6 reads from `eval_results`.

## Tuning loop

Judge prompt and dimension rubrics live in `lakeaudit/eval/judge_rubric.yaml`:
...
Tweak between dry runs to make the judge consistent. Run `lakeaudit eval --n 20 --dry-run` for fast iteration.

## Self-test

Before each eval run, the harness runs the judge on a **canonical pair** (committed to `tests/fixtures/eval/canonical_pair.json`) — a hand-curated "clearly LakeAudit wins" case where the LakeAudit record has rich methodology tags and the GPT-4 record has vague filler.

If the judge calls this canonical pair anything other than "LakeAudit wins, methodology delta ≥ +2", abort the eval run and surface a warning. This catches:
