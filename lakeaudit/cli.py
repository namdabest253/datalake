...
Subcommands:
    lakeaudit ingest <path>     Walk a folder, parse PDFs, insert documents.
    lakeaudit run               Run the 6-pass loop on INGESTED docs.
    lakeaudit eval --n 200      Side-by-side LakeAudit vs single-pass GPT-4.
    lakeaudit export            Emit JSONL + CSV + dataset_card.
    lakeaudit dashboard         Launch the Streamlit UI.
"""
...
def ingest(
    ...
    raise NotImplementedError("TODO: wire to lakeaudit.ingest.parser + storage.db")
...
def run(
    ...
    raise NotImplementedError("TODO: wire to lakeaudit.loop.state_machine")
...
def eval_cmd(
    ...
    raise NotImplementedError("TODO: wire to lakeaudit.eval.harness")


@app.command()
def export(
    run_id: str | None = typer.Option(None, "--run-id"),
    out: Path = typer.Option(Path("./.lakeaudit/export"), "--out"),
) -> None:
    """Emit JSONL + catalog CSV + dataset_card.md."""
    raise NotImplementedError("TODO: wire to lakeaudit.export.jsonl/csv/dataset_card")
...
def dashboard(
    ...
    """Launch the Streamlit UI. Thin wrapper over `streamlit run lakeaudit/dashboard/app.py`."""
